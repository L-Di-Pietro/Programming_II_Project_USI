...
export async function exportReportPdf(runId, report) {
    ...
    const images = await Promise.all(figures.map((f) => Plotly.toImage({ data: f.figure.data, layout: f.figure.layout }, { format: "png", width: 1000, height: 520, scale: 2 })));
...
function drawCover(doc, y, c) {
    ...
    if (c.demoMode) {
        ...
        doc.setFontSize(9);
        doc.text("Demo mode: report was generated by the NullProvider (no real LLM call).", MARGIN, y);
        y += 14;
    }
    return y + 12;
}
function drawChart(doc, y, label, pngDataUrl) {
...
function drawHeading(doc, y, text, size) {
...
function drawMarkdown(doc, y, md) {
...
function drawDisclaimerFooterOnAllPages(doc) {
    ...
    for (let i = 1; i <= pages; i++) {
        ...
        doc.setFontSize(8);
        doc.setTextColor(COLOR_MUTED);
        doc.text("AI analysis is not financial advice. Past performance does not guarantee future results.", MARGIN, PAGE_H - MARGIN + 6);
