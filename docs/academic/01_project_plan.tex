...
The system delivered for the 2026-06-12 submission (\texttt{v1.0.0})
provides:
...
\begin{itemize}
  ...
  \item Ten strategies across four families (trend, momentum, breakout,
        mean-reversion), each implementing a uniform signal contract.
  \item An event-driven backtest engine that enforces look-ahead-bias prevention by
        construction.
  \item Configurable commission (bps), slippage (bps), and fixed-fraction
        position sizing.
...
production-leaning quality the rubric expects and the timeline the
academic calendar allows:

\begin{itemize}
  \item \textbf{Python 3.11+ / FastAPI / SQLAlchemy 2 / Pydantic v2} on the backend.
        FastAPI gives us OpenAPI for free (the live \texttt{/openapi.json} schema is
        the source of truth for the typed frontend client); SQLAlchemy's generic
        types let the same schema run on SQLite in development and PostgreSQL in
        production via a single \texttt{DATABASE\_URL} switch.
  \item \textbf{React 18 + TypeScript + Vite + TailwindCSS + Plotly.js} on the
        frontend: interactive charts without manual SVG plumbing, and compile-time
        schema checking against the OpenAPI-typed client.
  ...
  \item \textbf{LLM via a provider abstraction:} \texttt{LLMProvider} with a
        deterministic \texttt{NullProvider} (used by tests) and an opt-in
        \texttt{GeminiProvider}; LLM features are off by default.
\end{itemize}

\subsection{Project timeline}

The project ran from 2026-04-25 (initial commit) to 2026-06-12 (final submission),
staffed by four contributors. The first four weeks built a feature-complete
release candidate; the rest hardened, deployed, and polished it to the deadline:
...
\begin{itemize}
  ...
  \item \textbf{Week~4 \textemdash{} 2026-05-18 to 2026-05-24.} Data-layer
        ...
        expansion from 5 to 10; benchmark overlays; shared crosshair and legend;
        CI workflows; interim documentation pass.
  \item \textbf{Weeks~5--7 \textemdash{} 2026-05-25 to 2026-06-12.} Deployment and
        operational hardening (one-command launcher, Docker/Railway, per-browser
        run isolation); a print-quality headless-Chromium PDF pipeline; the
        QuantEdge rebrand; a nightly documentation-refresh workflow; and the final
        results/screenshots/recompile pass.
\end{itemize}
