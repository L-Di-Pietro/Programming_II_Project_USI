...
frontend code. Adding the sixth-through-tenth strategies (PR~\#7) cost zero
frontend work as a result \textemdash{} the single biggest productivity
multiplier in the project.

\paragraph{The five-agent abstraction.} Splitting the runtime into named agents,
each inheriting \texttt{BaseAgent[TIn, TOut]} with a single public
\texttt{run(payload)} method, gave us seam-by-seam testability and kept the LLM
surface isolated behind one agent. The boundary between the one LLM-backed
agent and the four deterministic agents was the right boundary: it kept the test
