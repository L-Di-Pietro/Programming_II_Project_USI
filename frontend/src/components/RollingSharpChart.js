...
function rollingMean(arr, i, w) {
    return arr.slice(i - w + 1, i + 1).reduce((a, b) => a + b, 0) / w;
}
function rollingStd(arr, i, w, mean) {
    const slice = arr.slice(i - w + 1, i + 1);
    return Math.sqrt(slice.reduce((a, b) => a + (b - mean) ** 2, 0) / w);
}
export function RollingSharpChart({ equityData }) {
    if (!equityData || equityData.length < WINDOW + 2) {
        return _jsx(ChartEmpty, { label: "Not enough data for rolling Sharpe (need 43+ bars)." });
    }
